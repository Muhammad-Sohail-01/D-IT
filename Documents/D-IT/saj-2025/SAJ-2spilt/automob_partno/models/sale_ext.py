...
class SaleOrder(models.Model):
    ...
    warranty_contract_id = fields.Many2one(related='job_card_id.warranty_contract_id', string="Warranty Contract")
    miles = fields.Integer(
        string="Kilometers",
        related='job_card_id.miles',
        store=True,
        readonly=False
    )
    ...
    def _compute_count(self):
        for record in self:
            ...
            record.unselect_service_count = self.env['sale.order.line'].sudo().search_count(
                [('order_id', '=', record.id),
                 ('active', '=', False),
                 ('display_type', '=', False)])
    ...
    def action_view_unselect_services(self):
        action = {
            'type': 'ir.actions.act_window',
            'name': 'Unselected Services / Parts',
    ...
    def action_confirm(self):
        self.ensure_one()
        if not self.partner_sale_order:
            warranty_lines = self.order_line.filtered(lambda l: l.is_warranty)
            warranty_qty = sum(warranty_lines.mapped('product_uom_qty'))

            if warranty_qty > 1 or len(warranty_lines) > 1:
                return {
                    'type': 'ir.actions.act_window',
                    'name': _ ('Warranty Confirmation'),
                    'res_model': 'warranty.confirmation.wizard',
                    'view_mode': 'form',
                    'target': 'new',
    ...
    #
    # def write(self, vals):
    #     print("print lllllllllllllllllllllll", vals)
