...
class ProductTemplate(models.Model):
    ...
    def _update_partno_lines(self):
        """
        Synchronize all partno_line_id entries in product.template with related product.product records.
        Create a corresponding line in product.product for each part line in product.template.
        """
        for template in self:
            # Get all related products for the product template
            related_products = self.env['product.product'].search([('product_tmpl_id', '=', template.id)])
            ...
            for partno_line in template.partno_line_id:
                for product in related_products:
                    ...
                    if existing_partno:
                        ...
                        self.env['auto.mobile.product.product.partno.line'].create({
                            'partno': partno_line.partno,
                            'partno_id': template.id,
                            'partno_product_id': product.id,
                            'part_internal_no': partno_line.part_internal_no,
        ...
        # Call the method to update `estimate_time`
    ...
    def create(self, vals):
        ...
        # Synchronize part lines after creation
        record._update_partno_lines()
    
        return record
    
    
    def write(self, vals):
        # Update the product.template record
        result = super(ProductTemplate, self).write(vals)
    
        # Synchronize part lines after update
        self._update_partno_lines()
