...
class VehicleInspection(models.Model):
    ...
    def _expand_groups(self, states, domain, order):
        return ['draft', 'inspection_walkthrough', 'concern', 'concern_approve', 'concern_reject',
                'quotation',
                'approve', 'reject', 'parts_request', 'parts_available', 'in_repair',
    ...
    def action_send_update_quot(self):
        additional_part = self.inspection_required_parts_ids.filtered(
            lambda
                line: line.is_additional_part and line.additional_part_status == 'pending').mapped(
            'id')
        additional_services = self.inspection_required_service_ids.filtered(
            lambda line: line.is_additional_service and line.additional_service_status == 'pending')
        data = {
        ...
        self.env['mail.template'].sudo().browse(template_id.id).with_context(ctx).send_mail(self.id,
                                                                                            force_send=True)
