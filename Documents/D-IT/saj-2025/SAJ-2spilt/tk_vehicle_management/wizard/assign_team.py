...
class AssignTeam(models.TransientModel):
    ...
    team_id = fields.Many2one('inspection.team', string="Team", default=lambda self: self._get_default_team(), store=True)
    ...
    leaders_ids = fields.Many2many('hr.employee', string="Team Leaders", default=lambda self: self._get_default_leaders(),
    ...
    def _get_default_team(self):
        """Fetch the default team for the user's company"""
        return self.env['inspection.team'].search([('default_team', '=', True),
                                                   ('company_id', '=', self.env.company.id)], limit=1).id


    @api.model
    def _get_default_leaders(self):
        """Fetch employees who are default leaders in the current company"""
        return self.env['hr.employee'].search([
            ('default_team_leader', '=', True),
