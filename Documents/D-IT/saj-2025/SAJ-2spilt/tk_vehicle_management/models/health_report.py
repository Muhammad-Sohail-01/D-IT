...
class VehicleRequiredServices(models.Model):
    ...
    total_amount = fields.Monetary(string="Total Amount", compute="_compute_total_amount", store=True, readonly=True)
    ...
    estimate_time = fields.Float(string="Estimate Time", store=True)
    task_id = fields.Many2one('project.task', string="Task")
    ...
    def onchange_product_info(self):
        for rec in self:
            ...
            self.price = rec.product_id.list_price
            self.estimate_time = rec.product_id.estimate_time
            self.tax_ids = rec.product_id.taxes_id.filtered (
                lambda t: t.type_tax_use == 'sale' and t.company_id == self.company_id
            )

    @api.onchange('price', 'qty', 'estimate_time', 'nos')
    def onchange_price_qty_time(self):
       for rec in self:
           if rec.nos:
               rec.qty = rec.estimate_time * rec.nos
    ...
    def _compute_total_amount(self):
       for rec in self:
           ...
           if rec.price != rec.product_id.list_price:
               ...
               product_estimate_time = rec.product_id.estimate_time
           ...
           rec.total_amount = round(rec.total_amount)
                ...
                for service_record in self:
                    ...
                    if sale_line:
                        ...
                        sale_line.write({
                            ...
                            'estimate_time':service_record.estimate_time,
    ...
    def action_archive(self):
        for record in self:
            ...
            record.write({'service_selected': False})
            sale_order = record.inspection_id.sale_order_id
            if sale_order:
                sale_line = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id),
                ...
                sale_line.write({'active': False})

    def action_unarchive(self):
        for record in self:
            record.write({'active': True})
            record.write({'service_selected': True})
            sale_order = record.inspection_id.sale_order_id
            if sale_order:
                sale_line = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id),
                ...
                sale_line.write({'active': True})

    @api.model
    def create(self, vals_list):
        if vals_list.get('display_type'):
            vals_list.update(product_id=False, qty=0, uom_id=False)
...
class VehicleRequiredParts(models.Model):
    ...
    def action_archive(self):
        for record in self:
            record.write({'active': False})
            record.write({'part_selected': False})
            sale_order = record.inspection_id.sale_order_id
            if sale_order:
                sale_line = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id),
    ...
    def action_unarchive(self):
        for record in self:
            record.write({'active': True})
            record.write({'part_selected': True})
            sale_order = record.inspection_id.sale_order_id
            if sale_order:
                sale_line = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id),
    ...
    def create(self, vals_list):
        if vals_list.get('display_type'):
            vals_list.update(product_id=False, qty=0, uom_id=False, service_id=False)
        return super().create(vals_list)
