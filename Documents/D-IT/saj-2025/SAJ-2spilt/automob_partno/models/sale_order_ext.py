...
class SaleOrderLine(models.Model):
    ...
    brand_id = fields.Many2one('fleet.vehicle.model.brand', string="Brand",
                               domain=[('part_no_id', '=', 'product_template_id')])
    ...
    available_brand_ids = fields.Many2many(
        'fleet.vehicle.model.brand', 'available_bra_rel',
    ...
    warranty_expiry_date = fields.Date(string="Warranty Expiry Date", compute="_compute_warranty_expiry_date",
    ...
    def _onchange_product_id(self):
        if self.product_id:
            self.is_warranty = self.product_id.is_warranty

    @api.onchange('product_id')
    def _onchange_product_partner_sale(self):
        if self.product_id and self.product_id.partner_sale_only:
            if not self.order_id.partner_sale_order:
                raise ValidationError(_("Product %s can only be sold to partners.") % self.product_id.name)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('product_id'):
                ...
                if product.partner_sale_only and not self.env['sale.order'].browse(
                        vals['order_id']).partner_sale_order:
                    raise ValidationError(_("Product %s can only be sold to partners.") % product.name)
    ...
    #             service =
    #             inspection.inspection_required_service_ids.filtered(lambda s: s.product_id == line.product_id)
    #             if service:
