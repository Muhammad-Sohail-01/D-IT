...
class PartNo(models.Model):
    ...
    _rec_name = 'partno'
    _rec_names_search = ['partno', 'partno_product_id']
    ...
    partno = fields.Char(string="Part No", required=True)
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            part_type_label = dict(rec._fields['part_type'].selection).get(rec.part_type, '')
            rec.display_name = f"{rec.partno}-{rec.partno_product_id.name}[{part_type_label}]"


    @api.constrains('partno', 'part_type', 'brand_ids')
    def _check_duplicate_partno(self):
        for rec in self:
            existing = self.search([
                ('partno', '=', rec.partno),
                ('part_type', '=', rec.part_type),
                ('brand_ids', '=', rec.brand_ids.ids),
                ('id', '!=', rec.id)  # Exclude current record in case of update
            ])
            if existing:
                raise ValidationError("A record with the same Part No, Part Type, and Brand already exists!")
