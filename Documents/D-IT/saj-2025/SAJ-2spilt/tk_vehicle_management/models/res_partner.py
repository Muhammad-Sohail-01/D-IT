...
class RegisterVehicle(models.Model):
    ...
    vehicle_history_notes = fields.Html (string="Vehicle History", readonly=True,
                                         help="Shows history of all job cards for this vehicle")
    ...
    last_km_update = fields.Integer (string="Last KM Update", readonly=True)
    ...
    def _update_vehicle_history(self):
        # Get all job cards for this vehicle
        job_cards = self.search ([
                ('register_vehicle_id', '=', self.register_vehicle_id.id),
                ('id', '!=', self._origin.id)
        ], order='create_date desc')

        history_html = ['<div class="vehicle-history">']
        for job in job_cards:
            history_html.append (f'''
                    <div class="job-card-entry">
                        <strong>Job Card:</strong> {job.name}<br/>
                        <strong>Date:</strong> {job.inspection_date}<br/>
                        <strong>Status:</strong> {dict (self._fields['status'].selection).get (job.status)}<br/>
                        <strong>Kilometers:</strong> {job.miles or 'N/A'}<br/>
                        <strong>Services:</strong> {', '.join (job.inspection_required_service_ids.mapped ('name'))}<br/>
                        <hr/>
                    </div>
                ''')
        history_html.append ('</div>')

        self.vehicle_history_notes = ''.join (history_html)
