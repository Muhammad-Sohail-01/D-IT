...
def send_quotation_user_action_mail(values):
    ...
    template_id = request.env.ref('tk_vehicle_management.customer_quotation_action_part_mail_template').sudo()
    email_values = {
    ...
    request.env['mail.template'].sudo().browse(template_id.id).with_context(ctx).send_mail(inspection_id,
...
class VehicleServiceQuotation(http.Controller):
    @http.route(["/vehicle-service/job-card/", "/vehicle-service/job-card/<string:access_token>"], type='http',
                auth="public", website=True)
    def user_quotation(self, access_token=None, **kw):
        quotation_template = 'tk_vehicle_management.user_quot_details'
        quot_term = request.env['ir.config_parameter'].sudo().get_param('tk_vehicle_management.quot_term')
    ...
    @http.route('/vehicle-service/<string:type>/<string:token>/<string:report>', type='http', auth='public')
    def user_job_card_report(self, token, type, report):
        inspection_id = request.env['vehicle.inspection'].sudo().search([('access_token', '=', token)])
        if not inspection_id:
    ...
    def job_card_quot_service(self, **kw):
        ...
        inspection_id = request.env['vehicle.inspection'].sudo().search([('access_token', '=', kw.get('access_token'))])
        ...
        user_services_ids = selected_services
    ...
    @http.route(["/vehicle/job-card/quotation/<string:access_token>/accept"], type='json', auth="public", website=True)
    def job_card_quote_accept(self, access_token=None, **kw):
        redirect_url = "/vehicle-service/job-card/%s" % access_token
        inspection_id = request.env['vehicle.inspection'].sudo().search([('access_token', '=', access_token)])
        inspection_id.write({
    ...
    @http.route(["/vehicle-service/job-card/reopen-request/<string:access_token>"], type='http', auth="public",
                website=True)
    def user_quotation_quote_reopen_request(self, access_token=None, **kw):
        redirect_url = "/vehicle-service/job-card/%s" % access_token
        inspection_id = request.env['vehicle.inspection'].sudo().search([('access_token', '=', access_token)], limit=1)
    ...
    @http.route('/vehicle-service/job-card/quot-cancel/<string:access_token>', type='http', auth='public')
    ...
    @http.route('/vehicle-service/job-card/additional-part/submit', type='http', auth='public')
    def user_additional_spare_parts(self, **kw):
        ...
        # # Default Selected Parts
        # for data in additional_part_id.additional_part_ids:
        #     if data.required_status == 'must_required':
        #         data.additional_part_selected = True
    ...
    @http.route(["/vehicle/job-card/additional-part/quotation/<string:access_token>/accept"], type='json',
                auth="public", website=True)
    ...
    @http.route(["/vehicle-service/job-card/additional-part/quotation/<string:access_token>/cancel"], type='http',
                auth="public", website=True)
    ...
    @http.route(['/my/vehicle', '/my/vehicle/<string:type>'], type='http', auth="user", website=True)
    def portal_vehicle_tree(self, type=None, **kwargs):
        inspection_obj = request.env['vehicle.inspection'].sudo()
        template = None
        values = {}
        domain = [('customer_id', '=', request.env.user.partner_id.id)]
        if not type or type not in ['jc-updated-quot', 'job-card-quot', 'vehicle-warranty', 'registered-vehicle',
                                    'job-cards']:
    ...
    @http.route(['/my/vehicle/<string:filter_by>/<string:access_token>'], type='http', auth="user", website=True)
    def portal_vehicle_tree_filtered(self, filter_by=None, access_token=None, **kwargs):
        ...
        if filter_by == 'filter-warranty':
            vehicles = request.env['register.vehicle'].sudo().search(
                ...
                limit=1).mapped('id')
            warranties = request.env['vehicle.warranty'].sudo().search([('register_vehicle_id', 'in', vehicles)])
            values = {
                'warranties': warranties,
                'page_name': 'portal_vehicle_warranty_view_tree'
            }
    ...
    @http.route(['/my/vehicle/registered-vehicle/form',
                 '/my/vehicle/registered-vehicle/form/<string:access_token>'], type='http', auth="user",
                website=True)
    def portal_register_vehicle_form(self, access_token=None, **kw):
        ...
        if not access_token:
            ...
            return request.render(template, values)
        # Check Record
        register_vehicle_id = request.env['register.vehicle'].sudo().search([('access_token', '=', access_token)],
                                                                            limit=1)
    ...
    @http.route(['/my/vehicle/vehicle-warranty/form',
                 '/my/vehicle/vehicle-warranty/form/<string:access_token>'], type='http', auth="user",
                website=True)
    def portal_vehicle_warranty_form(self, access_token=None, **kw):
        ...
        if not access_token:
            ...
            return request.render(template, values)
        # Check Record
        warranty_id = request.env['vehicle.warranty'].sudo().search([('access_token', '=', access_token)],
                                                                    limit=1)
        if not warranty_id:
            values['error'] = get_user_error('invalid_token')
            return request.render(template, values)
    ...
    @http.route(['/my/vehicle/jc-updated-quot/form',
                 '/my/vehicle/jc-updated-quot/form/<string:access_token>'], type='http', auth="user",
                website=True)
    ...
    def user_consent_info(self, access_token=None, **kw):
        ...
        if not access_token:
            ...
            return request.render(template, values)
        # Check Inspection Record
        inspection_id = inspection_obj.search([('consent_access_token', '=', access_token)], limit=1)
        if not inspection_id:
    ...
    # Consent Report
    @http.route('/vehicle-service/consent-report/<string:token>/Consent report', type='http', auth='public')
    ...
    # Consent Approve
    @http.route(["/vehicle-service/user-consent/<string:access_token>/accept"], type='json', auth="public",
                website=True)
    def job_card_consent_accept(self, access_token=None, **kw):
        redirect_url = "/vehicle-service/consent-approval/%s" % access_token
        inspection_id = request.env['vehicle.inspection'].sudo().search([('consent_access_token', '=', access_token)])
        inspection_id.write({
    ...
    @http.route(["/vehicle-service/user-consent/reopen-request/<string:access_token>"], type='http', auth="public",
                website=True)
...
class VehicleCustomerPortal(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        additional_part_quot_count = request.env['additional.part.confirmation'].sudo().search_count(
            [('inspection_id', 'in', job_card_ids)])
