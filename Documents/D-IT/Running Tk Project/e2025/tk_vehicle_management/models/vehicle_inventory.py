...
class VehiclePartsPicking(models.Model):
    ...
    def button_validate(self):
        ...
        if self.job_card_id:
            # Update the job card state based on the stock voucher state
            if self.state == "confirmed":
                self.job_card_id.write({'status': "waiting_for_parts"})
            elif self.state == "assigned":
                self.job_card_id.write({'status': "parts_available"})
            elif self.state == "done":
                # Check if all stock vouchers linked to the job card are done
                all_done = self.env['stock.picking'].search_count([
                    ('job_card_id', '=', self.job_card_id.id),
                    ('state', '!=', 'done')
                ]) == 0
                if all_done:
                    self.job_card_id.write({'status': "in_repair"})
    
            # Send validation email if necessary
