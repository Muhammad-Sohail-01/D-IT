...
class TaskAssign(models.TransientModel):
    ...
    def _onchange_task_title(self):
        for rec in self:
            if rec.service_ids:
                rec.task_title = self.inspection_id.name + ' Task : ' + ' + '.join(
                    rec.service_ids.mapped('name'))

    def action_assign_service_task(self):
        task_id = self.env['project.task'].create({
            'name': self.task_title,
            'allocated_hours': self.allocated_hours,
            'car_plate_number': self.registration_no,
            ...
            'date_deadline': self.inspection_id.estimate_delivery_date,
        })
