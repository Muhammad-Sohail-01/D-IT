        ...
        if self.name and reject == 'qc_check':
            ...
            body_inspection = Markup(
                'Task : <strong>' + str(
                    ...
                    self.name) + '</strong>')
            task_line_id.task_id.task_state = 'reject'
            task_line_id.task_id.message_post(body=body, message_type="notification",
                                              partner_ids=[self.env.user.partner_id.id])
            task_line_id.is_reject = True
            task_line_id.inspection_id.message_post(body=body_inspection, message_type="notification",
                                                    partner_ids=[self.env.user.partner_id.id])
