...
class VehicleHealthReport(models.Model):
    ...
    inspection_type = fields.Selection(related="inspection_id.inspection_type", string="Type of Inspection")
    ...
    untaxed_service_amount = fields.Monetary(string="Untaxed Service Amount", compute="_compute_total")
    ...
    def action_add_service_parts(self):  # Add Parts and Services from inspection tabs
        ...
        inspections = [
            ...
            (self.mechanical_condition, self.vehicle_mechanical_inspection_ids, 'Under Hood Services',
...
class VehicleRequiredServices(models.Model):
    ...
    vehicle_health_report_id = fields.Many2one('vehicle.health.report', string="Vehicle Health Report")
    ...
    required_status = fields.Selection([('must_required', 'Requires immediate attention'),
                                        ...
                                        ('okay', 'Checked and OK at this time')], default='must_required')
        ...
        for rec in self:
            ...
            if rec.product_id:
                total_tax_percentage = sum(
                    data.amount if data.amount_type != 'group' else sum(data.children_tax_ids.mapped('amount'))
...
class VehicleRequiredParts(models.Model):
    ...
    @api.depends('vehicle_health_report_id', 'vehicle_health_report_id.inspection_required_service_ids')
    def compute_services(self):
        for rec in self:
            rec.service_ids = rec.vehicle_health_report_id.inspection_required_service_ids.mapped('id')
    ...
    def action_view_forecast_report(self):
        ...
        if self.product_id:
            action['context']['warehouse'] = self.product_id.property_stock_inventory.warehouse_id.id
        return action
...
class VehicleInteriorCondition(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='in_product_rel',
                                   ...
                                   domain="[('detailed_type','=','service')]", string="Inner Conditions Service")
...
class VehicleOuterCondition(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='outer_product_rel',
                                   ...
                                   domain="[('detailed_type','=','service')]", string="Outer Condition Service")
...
class MechanicalConditionLine(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='mechanical_product_rel',
                                   column1='mechanical_part_id', column2='mechanical_product_id',
                                   domain="[('detailed_type','=','service')]", string="Mechanical Service")
...
class VehicleComponentsLines(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='components_product_rel',
                                   column1='components_part_id', column2='components_product_id',
                                   domain="[('detailed_type','=','service')]", string="Multi-Point Service")
...
class VehicleFluidsLine(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='fluid_product_rel',
                                   ...
                                   domain="[('detailed_type','=','service')]", string="Fluid Service")
    part_ids = fields.One2many(comodel_name='fluid.condition.parts', inverse_name='condition_id', string="Fluid Parts")
    required_attention = fields.Boolean()
    further_attention = fields.Boolean()
    okay_for_now = fields.Boolean()
    notes = fields.Text()
...
class UnderVehicleInspectionLine(models.Model):
    ...
    inspection_id = fields.Many2one('vehicle.health.report')
    service_ids = fields.Many2many(comodel_name='product.product', relation='under_vehicle_product_rel',
                                   column1='under_vehicle_part_id', column2='under_vehicle_product_id',
...
class TyreInspectionLine(models.Model):
    ...
    service_ids = fields.Many2many(comodel_name='product.product', relation='tyre_product_rel',
                                   ...
                                   domain="[('detailed_type','=','service')]", string="Tyre Service")
    part_ids = fields.One2many(comodel_name='tyre.condition.parts', inverse_name='condition_id', string="Tyre Parts")
...
class BrakeConditionLine(models.Model):
    ...
    inspection_id = fields.Many2one('vehicle.health.report')
    service_ids = fields.Many2many(comodel_name='product.product', relation='brake_condition_product_rel',
                                   column1='brake_condition_part_id', column2='brake_condition_product_id',
                                   domain="[('detailed_type','=','service')]", string="Service")
...
class InnerConditionsParts(models.Model):
    ...
    service_id = fields.Many2one('product.product', string="Service", domain="[('id','in',service_ids)]")
