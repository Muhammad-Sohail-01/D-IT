...
class VehicleInspection(models.Model):
    ...
    company_direct_quote_approval = fields.Boolean(string="is Driect Approval", related="company_id.is_direct_quote", store=True)
    ...
    #partner
    is_partner = fields.Boolean(string="As Partner")
    ...
    invoice_ids = fields.One2many(
        'account.move',  # Target model
        'job_card_id',   # Inverse field in account.move
        string="Invoices"
    )


    invoice_total = fields.Monetary(string="Invoice Total", compute="_compute_invoice_total",)

    @api.depends('invoice_ids.amount_total_signed')
    def _compute_invoice_total(self):
        for record in self:
            record.invoice_total = sum(record.invoice_ids.mapped('amount_total_signed'))
    ...
    total_due_amount = fields.Monetary(
        ...
        currency_field='currency_id',
        related="customer_id.total_due",
        readonly=True
    )
    is_quotation_sent_one = fields.Boolean(string="Quotation Sent", default=False)
    # quantity_matched = fields.Boolean(string="Quantity Matched")
    ...
    @api.constrains ('miles', 'register_vehicle_id')
    def _check_miles_increment(self):
        for record in self:
            if record.register_vehicle_id and record.miles:
                if record.miles < record.register_vehicle_id.last_km_update:
                    raise ValidationError (
                        _ ("New kilometer reading cannot be less than the previous reading (%s km)") %
                        record.register_vehicle_id.last_km_update)

    @api.onchange ('miles', 'register_vehicle_id')
    def _onchange_miles(self):
        if self.register_vehicle_id and self.miles:
            if self.miles < self.register_vehicle_id.last_km_update:
                self.miles = self.register_vehicle_id.last_km_update
                return {
                    'warning': {
                        'title'  : _ ('Warning'),
                        'message': _ ("Kilometer reading cannot be less than the previous reading (%s km)") %
                                   self.register_vehicle_id.last_km_update
                    }
                }

    @api.onchange ('register_vehicle_id')
    def onchange_vehicle_info(self):
        res = super ().onchange_vehicle_info ()
        if self.register_vehicle_id:
            self.miles = self.register_vehicle_id.last_km_update
        return res

    def write(self, vals):
        if 'miles' in vals and self.register_vehicle_id:
            if vals.get ('miles', 0) < self.register_vehicle_id.last_km_update:
                vals['miles'] = self.register_vehicle_id.last_km_update
        res = super ().write (vals)
        if 'miles' in vals and self.register_vehicle_id and vals.get ('miles'):
            self.register_vehicle_id.write ({
                'last_km_update': vals['miles']
            })
        return res
    ...
    def write(self, vals):
        ...
        self.inspection_required_service_ids.update_sale_order_lines()
        self.inspection_required_parts_ids.update_sale_order_lines()
    ...
    def _prepare_quotation_lines(self, service_ids=None, part_ids=None, is_additional=None):
        ...
        if service_ids:
            ...
            for service in service_ids:
                if service.price > 0 and service.uom_id:
                    quote_line_data = {
            ...
            if services_quote_lines:
                # Check if the section header already exists to avoid duplicates
                if not any(line[2].get('name') == 'Required Services' for line in quote_lines):
        ...
        if part_ids:
            ...
            for part in part_ids:
                if part.price > 0 and part.uom_id:
                    quote_line_data = {
                        ...
                        'warranty_applicable': part.warranty_applicable,
                    }
            ...
            if parts_quote_lines:
                # Check if the section header already exists to avoid duplicates
                if not any(line[2].get('name') == 'Required Parts' for line in quote_lines):
                    quote_lines.append((0, 0, {
                        'display_type': 'line_section',
    ...
    def action_create_required_service_part_sale_order(self):
        ...
        customer_id = self.partner_id if self.is_partner else self.customer_id
        ...
        if sale_order_id:
            ...
            # has_low_price = False
            # if sale_order_id:
            #     # Check for low prices in the order lines
            #     for line in sale_order_id.order_line:
            #         if line.price_unit < 2.00:
            #             has_low_price = True
            #             break

                # # Case 1: Only Services exist
                # if service_ids and not part_ids:
                #     if not has_low_price:
                #         self.status = 'price_verified'
                #         sale_order_id.state = 'payment_verified'
                #     else:
                #         raise ValidationError (_ ("Please check the service prices before processing."))

                # # Case 2: Only Parts exist
                # elif part_ids and not service_ids:
                #     if not has_low_price:
                #         self.status = 'price_verified'
                #         sale_order_id.state = 'payment_verified'
                #     else:
                #         raise ValidationError (_ ("Please check the part prices before processing."))

                # # Case 3: Both Services and Parts exist
                # elif service_ids and part_ids:
                #     if not has_low_price:
                #         self.status = 'price_verified'
                #         sale_order_id.state = 'payment_verified'
                #     else:
                #         raise ValidationError (_ ("Please check the prices before processing."))
    ...
    def action_status_quotation(self):
        ...
        self.status = 'sent'
        self.is_quotation_sent_one = True
    ...
    def action_status_concern(self):
        ...
        customer_id = self.partner_id if self.is_partner else self.customer_id
        mobile_no = self.check_whatsapp_phone(customer_id)
    ...
    def action_status_concern_approve(self):
        ...
        return {
            'type': 'ir.actions.client',
            'tag': 'reload',  # Reload the current screen
        }
    ...
    def action_status_concern_approve_manual(self):
        for record in self:
            if record.status == 'concern':
                record.status = 'concern_approve'
    ...
    def action_approve_quote(self):
        """
        Approve quote by creating sale order and updating status
        """
        # Check if direct quote is enabled for the company
        if not self.company_id.is_direct_quote:
            return {
                'type'  : 'ir.actions.client',
                'tag'   : 'display_notification',
                'params': {
                    'title'  : _ ('Quote Approval Restricted'),
                    'message': _ ('Direct quote approval is not enabled for this company.'),
                    'sticky' : False,
                }
            }

        # Ensure there are services or parts before approving
        if not self.inspection_required_service_ids and not self.inspection_required_parts_ids:
            return {
                'type'  : 'ir.actions.client',
                'tag'   : 'display_notification',
                'params': {
                    'title'  : _ ('No Services or Parts'),
                    'message': _ ('Please add services or parts before approving the quote.'),
                    'sticky' : False,
                }
            }

        # Create sale order and mark services/parts as created
        self.action_create_required_service_part_sale_order ()
    ...
    def action_skip_quote_email(self):
        """
        Skip quotation email process and directly move to approved state
        """
        # Find existing sale order
        sale_order = self.env['sale.order'].search ([('job_card_id', '=', self.id)], limit=1)

        if sale_order:
            # Update sale order state to sale
        ...
        self.status = 'approve'
        self.quote_state = 'signed'

        # Mark services and parts as selected
        self.inspection_required_service_ids.write ({'service_selected': True})
        self.inspection_required_parts_ids.write ({'part_selected': True})
    ...
    def action_create_invoice_from_sale_orders(self):
        ...
        self.invoice_id.register_vehicle_id = self.register_vehicle_id
    ...
    def action_open_invoice_wizard(self):
        """Open the default Odoo Invoice Wizard for this sale order"""
        self.ensure_one()

        if not self.sale_order_id:
            raise UserError("No Sale Order linked to this inspection.")
        ...
        return {
            ...
            'target': 'new',
            'context': {
                'default_sale_order_ids': [self.sale_order_id.id],
                'default_company_id': self.sale_order_id.company_id.id,
            },
        }
    ...
    def update_sale_order_lines(self):
        """
        Updates related sale.order.line records based on inspection_id and job_card_id.
        Triggered by a button click in the form view.
        """
        for record in self:
            if record.inspection_required_service_ids:
                sale_lines = self.env['sale.order.line'].search([
                    ('order_id.job_card_id', '=', record.id),
                    ('product_id', '=', record.inspection_required_service_ids.product_id.id)
                ])
                # # Gather details about sale_lines for the message
                # sale_lines_info = ", ".join([f"{line.id} - {line.name}" for line in sale_lines])
                
                # # Raise the error with details
                # raise ValidationError(f"The following sale order lines were found:\n{sale_lines_info}")
                for line in sale_lines:
                    line.write({
                        'product_uom_qty': record.inspection_required_service_ids.qty,
                        'price_unit': record.inspection_required_service_ids.price,
                        'price_subtotal': record.inspection_required_service_ids.untaxed_amount,
                        'tax_id': [(6, 0, record.inspection_required_service_ids.tax_ids.ids)],
                        'discount': record.inspection_required_service_ids.discount,
                    })
