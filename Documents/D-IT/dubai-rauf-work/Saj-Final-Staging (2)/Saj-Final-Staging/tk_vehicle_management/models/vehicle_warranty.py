...
class VehicleWarranty(models.Model):
    ...
    name = fields.Char(string='Warranty No', readonly=True, default=lambda self: _('New'), copy=False)
    ...
    color = fields.Char(related="register_vehicle_id.color", store=True)
    ...
    insurance_provider = fields.Char(related="register_vehicle_id.insurance_provider", store=True)
    ...
    warranty_limitation_ids = fields.Many2many('warranty.attributes', 'warranty_limitation_rel', 'warranty_id',
    ...
    def action_send_warranty_checkup_reminder(self):
        ...
        for rec in warranty_records:
            ...
            if delta.months > 6 and rec.next_reminder_date and rec.next_reminder_date == today_date:
                mail_template = self.env.ref('tk_vehicle_management.vehicle_warranty_reminder_mail_template')
                if mail_template:
...
class WarrantySaleOrder(models.Model):
    ...
    def _compute_warranty_count(self):
        for rec in self:
            rec.warranty_count = self.env['vehicle.warranty'].search_count([('sale_order_id', '=', rec.id)])
