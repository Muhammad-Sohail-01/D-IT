...
class SaleOrderLine(models.Model):
    ...
    part_nubmer_id = fields.Many2one('auto.mobile.product.product.partno.line', string="Part Number")
    ...
    def _onchange_part_nubmer_id(self):
        ...
        if self.part_nubmer_id.part_vendor_name:
    ...
    def _onchange_product_id(self):
        ...
        part_number = self.env['auto.mobile.product.product.partno.line'].search([
            ('partno_product_id', '=', self.product_id.id)
        ], limit=1)
    ...
    def _compute_available_brand_ids(self):
        for line in self:
            if line.product_id:
                ...
                partno_lines = self.env['auto.mobile.product.product.partno.line'].search(
