...
class VehicleHealthReport(models.Model):
    ...
    insurance_provider = fields.Many2one('res.partner', related="inspection_id.insurance_provider",
    ...
    leaders_ids = fields.Many2many('hr.employee', string="Team Leaders", domain="[('id','in',team_leaders_ids)]")
...
class VehicleRequiredServices(models.Model):
    ...
    qty = fields.Float(string="Qty", default=1.0, store=True)
    uom_id = fields.Many2one(related="product_id.uom_id", string="UOM")
    price = fields.Monetary(string="Unit Price")
    tax_ids = fields.Many2many('account.tax', string="Taxes")
    tax_amount = fields.Monetary(string="Tax Amount", compute="_compute_total_amount", store=True)
    untaxed_amount = fields.Monetary(string="Untaxed Amount", compute="_compute_total_amount", store=True)
    ...
    allowed_hour_change = fields.Boolean(string="Hours allowed change", related="product_id.allowed_hour_change")
    ...
    def _compute_hourly_cost(self):
        for record in self:
            if record.product_id:
                record.hourly_cost = record.product_id.list_price
            else:
    ...
    def onchange_product_info(self):
        ...
        # self.estimate_time = self.product_id.estimate_time
        self.tax_ids = self.product_id.taxes_id.filtered (
            lambda t: t.type_tax_use == 'sale' and t.company_id == self.company_id
        )

    @api.onchange('price', 'qty', 'estimate_time')
    def onchange_price_qty_time(self):
       for rec in self:
           if rec.price and rec.price != rec.product_id.list_price:
               # Use manual price for calculations
               if rec.estimate_time:
                   rec.total_amount = rec.estimate_time * rec.qty * rec.price
               else:
                   rec.total_amount = rec.qty * rec.price
                   
    @api.depends('qty', 'product_id', 'estimate_time', 'tax_ids', 'price', 'discount') 
    def _compute_total_amount(self):
       for rec in self:
           if not rec.product_id:
               continue
    
           if rec.price != rec.product_id.list_price:
               # Manual price calculations
               untaxed_amount = rec.price * rec.qty * (rec.estimate_time or 1)
           else:
               # Default product price calculations 
               product_estimate_time = rec.product_id.estimate_time or 0
               product_sales_price = rec.product_id.list_price or 0
               hourly_rate = rec.product_id.per_hour_price
               unit_price = hourly_rate * max(rec.estimate_time or product_estimate_time, 1)
               untaxed_amount = unit_price * rec.qty
    
           tax_amount = sum((untaxed_amount * tax.amount / 100) for tax in rec.tax_ids)
           rec.untaxed_amount = untaxed_amount
           rec.tax_amount = tax_amount 
           rec.total_amount = untaxed_amount + tax_amount

           if rec.discount:
                discount_value = (rec.discount / 100) * rec.total_amount
                rec.total_amount -= discount_value
    ...
    # if rec.discount:
            #     rec.total_amount -= (rec.discount / 100) * rec.total_amount
    # @api.depends ('qty', 'product_id', 'price', 'tax_ids', 'estimate_time', 'tax_ids.amount', 'discount', 'hourly_cost')
    # def _compute_total_amount(self):
    #     for rec in self:
    #         if not rec.product_id:
    #             continue
    #
    #         # Get product's configured estimate time and sales price
    #         product_estimate_time = rec.product_id.estimate_time or 0
    #         product_sales_price = rec.product_id.list_price or 0
    #
    #         # Calculate hourly rate
    #         if product_estimate_time >= 1:
    #             hourly_rate = product_sales_price / product_estimate_time
    #         else:
    #             hourly_rate = product_sales_price  # Use full price as hourly rate if estimate time < 1
    #
    #         # Calculate total estimate time based on quantity
    #         total_estimate_time = rec.estimate_time * rec.qty if rec.estimate_time else 0
    #         rec.estimate_time = total_estimate_time / rec.qty if rec.qty else 0
    #
    #         # Calculate unit price based on estimate time
    #         rec.price = hourly_rate * (rec.estimate_time if rec.estimate_time else product_estimate_time)
    #
    #         # Calculate untaxed amount
    #         untaxed_amount = rec.price * rec.qty
    #
    #         # Calculate tax
    #         tax_amount = sum (
    #             (untaxed_amount * tax.amount / 100)
    ...
    @api.onchange ('qty', 'estimate_time')
    def onchange_qty_estimate_time(self):
    ...
    @api.model
    def action_add_additional_services(self, vals):
        """Add Additional Parts: Always create a new line with `is_additional=True`."""
        self.env['vehicle.required.services'].create({
            'inspection_id': self.inspection_id.id,
    ...
    def update_sale_order_lines(self):
        for record in self:
            if record.inspection_id and record.inspection_id.sale_order_id.job_card_id:
                sale_order = record.inspection_id.sale_order_id
                # Get all current product IDs in the service model
                current_product_ids = self.mapped('product_id.id')

                # Get all sale order lines for the corresponding sale order
                sale_lines = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id)
                ])

                # Update or create sale order lines for products in the service model
                for service_record in self:
                    sale_line = sale_lines.filtered(lambda line: line.product_id == service_record.product_id)
                    if sale_line:
                        # Update existing sale order line
                        sale_line.write({
                            'product_uom_qty': service_record.qty,
                            'price_unit': service_record.price,
                            'price_subtotal': service_record.untaxed_amount,
                            'tax_id': [(6, 0, service_record.tax_ids.ids)],
                            'discount': service_record.discount,
                            'warranty_applicable': service_record.warranty_applicable,
                        })

    @api.onchange('product_id')
    def update_sale_order_lines_product(self):
        for rec in self:
            if not rec.inspection_id or not rec.inspection_id.sale_order_id:
                continue
            ...
            sale_order = rec.inspection_id.sale_order_id

            # Get the previous value of product_id
            previous_product_id = rec._origin.product_id.id if rec._origin else None
            new_product_id = rec.product_id.id if rec.product_id else None

            if previous_product_id and previous_product_id != new_product_id:
                # Search for the sale order line with the previous product_id
                ...
                if sale_line:
                    # Update the product_id in the sale order line
                    sale_line.sudo().write({
                        'product_id': new_product_id,
                        'name': rec.product_id.display_name,  # Update description if needed

                    })
                    print(
                        f"Updated Sale Order Line: {sale_line.id}, Product changed from {previous_product_id} to {new_product_id}")
            else:
                self.env['sale.order.line'].create({
                    ...
                    'price_subtotal': rec.untaxed_amount,
                    'tax_id': [(6, 0, rec.tax_ids.ids)],
                    'discount': rec.discount,
                })
                print("No product change detected or invalid previous product.")
...
class VehicleRequiredParts(models.Model):
    ...
    def onchange_product_info(self):
        ...
        if self.product_id:
            self.tax_ids = self.product_id.product_tmpl_id.taxes_id.filtered(
                lambda t: t.type_tax_use == 'sale' and t.company_id == self.company_id
            )

    @api.depends('product_id', 'qty', 'discount', 'price', 'tax_ids', 'tax_ids.amount')
    def _compute_total_amount(self):
        for rec in self:
            total = 0.0
            tax_amount = 0.0
            ...
            if rec.discount:  # Ensure fields are not None
                discount_value = (rec.discount / 100) * rec.total_amount
                rec.total_amount = rec.total_amount - discount_value
            else:
                rec.total_amount = rec.tax_amount + rec.untaxed_amount
    ...
    def action_add_additional_parts(self):
        """Add Additional Parts, ensuring no duplicates and handling deleted records."""
        for record in self:
            # Check if the record still exists
            if not record.exists():
                continue  # Skip if the record was deleted

            # Check if a similar record already exists
            existing_record = self.env['vehicle.required.parts'].search([
                ('inspection_id', '=', record.inspection_id.id),
                ('vehicle_health_report_id', '=', record.vehicle_health_report_id.id),
                ('part_selected', '=', True),
                ('is_additional', '=', True)
            ], limit=1)
            ...
            if not existing_record:
                ...
                # If the record exists but is deleted, we should check if it's really gone
    ...
    def update_sale_order_lines(self):
        for record in self:
            if record.inspection_id and record.inspection_id.sale_order_id.job_card_id:
                sale_order = record.inspection_id.sale_order_id
                # Get all current product IDs in the service model
                current_product_id = self.mapped('product_id.id')

                # Get all sale order lines for the corresponding sale order
                sale_lines = self.env['sale.order.line'].search([
                    ('order_id', '=', sale_order.id)
                ])
                for parts_record in self:
                    sale_line = sale_lines.filtered(lambda line: line.product_id == parts_record.product_id)
                    if sale_line:
                        # Update existing sale order line
                        sale_line.write({
                            'product_uom_qty': parts_record.qty,
                            'price_unit': parts_record.price,
                            'price_subtotal': parts_record.untaxed_amount,
                            'tax_id': [(6, 0, parts_record.tax_ids.ids)],
                            'discount': parts_record.discount,
                            'warranty_applicable': parts_record.warranty_applicable,
                        })
