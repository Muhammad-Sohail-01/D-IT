...
class AssignTeam(models.TransientModel):
    ...
    technician_ids = fields.Many2many('res.users', 'assign_technician_rel', 'assign_technician_id',
                                      'assign_user_technician_id', string="Technicians",
    ...
    def action_assign_team(self):
        active_id = self._context.get('active_id')
        inspection_id = self.env['vehicle.inspection'].browse(active_id)
        if not inspection_id.vehicle_health_report_id:
